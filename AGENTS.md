...
- **API HTTP implementada com Axum 0.8.** `src/main.rs` carrega o `.env` (dotenvy), inicializa `tracing` (logs JSON), cria o `PgPool`, monta o `router()` e chama `server(app)`. Rotas ativas: `GET /health`, `GET /ready`, `GET/POST /ff-codex/games`.
- **SQLx integrado ao GET** — `main.rs` cria `PgPool` + `GameRepository` + `GameService` e injeta via `AppState` no `router()`. `GET /ff-codex/games` lê do banco; `GET /ff-codex/games?titulo=...` filtra por título (busca parcial case-insensitive via `ILIKE`); sem filtro → lista completa; sem match → `200 []`. `POST /ff-codex/games` ainda **não** persiste — devolve um eco do payload. Próxima etapa: INSERT no POST. Não inventar endpoints além dos existentes.
...
- **Camadas `domain/`, `repository/` e `service/`:** `domain/game.rs` define a struct `Game` (`#[derive(FromRow, Debug)]`, campos `pub` `id`/`titulo`/`ano_lancamento`; `id` tem `#[expect(dead_code)]` pois ainda não é exposto na API); `repository/game.rs` define `GameRepository { pool: PgPool }` (`#[derive(Clone)]`) com `all_games() -> Result<Vec<Game>, sqlx::Error>` via `select * from games` e `games_by_titulo(titulo) -> Result<Vec<Game>, sqlx::Error>` via `select * from games where titulo ilike '%' || $1 || '%'`; `service/game_service.rs` define `GameService` com `all_games()` e `games_by_titulo()` — ambos retornam `Result<Vec<Game>, GameError>` (thiserror, `Internal(#[from] sqlx::Error)`) — **retornam domínio, nunca DTO**; o `service` não importa `rest::dto`. A conversão `Game` → `GamesResponse` é feita **no handler** (`map(GamesResponse::from)`) via `impl From` em `rest/dto/game.rs`. Consultas vão no repositório, nunca no handler.
